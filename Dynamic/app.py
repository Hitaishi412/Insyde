...
if uploaded_file is not None:
    try:
        ...
        display_results = results[
            ["user", "anomaly_score", "risk_score", "risk_level", "reason"]
        ...
        # Export static image to disk immediately after rendering
        risk_fig.write_image("risk_distribution.png", width=600, height=400, scale=2)

        # --------------------------
        # Top 10 Risk Users Bar Chart
        # --------------------------
        st.subheader("🚨 Top 10 Highest Risk Users")
        top_users = results.sort_values("risk_score", ascending=False).head(10)
        ...
        top_users_fig.update_layout(xaxis_title="Employee", yaxis_title="Risk Score")
        st.plotly_chart(top_users_fig, use_container_width=True)

        # Export static image to disk immediately after rendering
        top_users_fig.write_image("top_users.png", width=600, height=400, scale=2)
        ...
        # --------------------------
        # High Risk Users Dataframe
        # --------------------------
        st.subheader("🚨 High Risk Users Details")
        if len(high_risk) > 0:
            st.dataframe(
                high_risk[["user", "anomaly_score", "risk_score", "risk_level", "reason"]]
                .sort_values("risk_score", ascending=False)
            )
        else:
        ...
        if len(high_risk) > 0:
            ...
            st.success("No critical insider threat indicators detected. Continue routine monitoring.")

        # --------------------------
        # Export/Download Action Options
        # --------------------------
        st.subheader("📥 Export Threat Assessment Reports")
        ...
        st.error(f"Error Processing Data Pipeline: {str(e)}")
